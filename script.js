...
// Robust fallback: ensure Expand/Collapse all always works even if DOM changes.
document.addEventListener('click', (e) => {
  const el = e.target.closest('#expandAll, #collapseAll, [data-action="expand-all"], [data-action="collapse-all"]');
  if(!el) return;
  if(el.matches('#expandAll, [data-action="expand-all"]')){
    e.preventDefault();
    document.querySelectorAll('.rec-details').forEach(d => d.open = true);
  }
  if(el.matches('#collapseAll, [data-action="collapse-all"]')){
    e.preventDefault();
    document.querySelectorAll('.rec-details').forEach(d => d.open = false);
  }
}, {passive:false});
